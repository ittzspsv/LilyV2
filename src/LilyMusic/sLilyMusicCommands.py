'''
CODE NOT WRITTEN BY SHREE
DIRECTLY EXTRACTED FROM Lavalink Client Sample
CODE REMODIFIED BY SHREE
'''
...
class Music(commands.Cog):
    ...
    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.CommandInvokeError):
            await ctx.send(embed=mLily.SimpleEmbed(f"{error.original}", 'cross'))
        ...
        if voice_client is None:
            ...
            if voice_channel.user_limit > 0:
                ...
                if len(voice_channel.members) >= voice_channel.user_limit and not ctx.me.guild_permissions.move_members:
                    raise commands.CommandInvokeError('The Voice Channel you`re in is full!')
    ...
    @commands.hybrid_command(name='lowpass', description='lowpass filter')
    @commands.check(create_player)
    ...
    @commands.hybrid_command(name='disconnect', description='Disconnects the player from the voice channel and clears its queue.')
    @commands.check(create_player)
