...
class LilyBloxFruits(commands.Cog):
    ...
    async def delete_combo(self, ctx: commands.Context, combo_id: str):
        try:
            ...
            if row:
                await ctx.reply(embed=LilyEmbed.simple_embed(f"Successfully deleted combo id {row}"))
            else:
                await ctx.reply(embed=LilyEmbed.simple_embed(f"Cannot find the combo ID", 'cross'))
        except Exception as e:
